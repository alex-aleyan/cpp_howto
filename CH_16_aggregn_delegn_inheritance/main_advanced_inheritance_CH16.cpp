//	CH16 ADVANCED INHERITANCE:
//		-	AGGREGATION (HAS-A): Declaring an object as a member of a new class contained by this new class.
//		-	DELEGATION (): Using the member methods of the AGGREGATED CLASS by the containing class.
//		-	PRIVATE INHERITANCE
//		-	IMPLEMENTING (ONE CLASS) IN TERMS OF (ANOTHER CLASS): Building one class on the capabilities of another without using public inheritance (by using protected/private inheritance).

//	#1	AGGREGATION/CONTAINMENT (HAS-AN RELATIONSHIP; perhaps HAS-AN OBJECT): AN OBJECT OF ONE CLASS CONTAINED WITHIN ANOTHER CLASS AS A MEMBER DATA
//		The disadvantage of aggregation is that it increases the number of object to be created when the aggregating class is used to create an object.
//		EXAMPLE:
//			Class Name { };
//			Class Address {};
//			Class Employee { 
//							public: 
//								GetName(){return EmpName;} 
//								GetAddress(){return EmpAddress;} 
//							private: 
//								Name EmpName;		// <- AGGREGATION
//								Address EmpAddress;	// <- AGGREGATION
//							}
//	#2	ACCESSING DATA MEMBERS WHEN USING AGGREGATED CLASSES:
//		-	For an object of the aggregating class to access the data of the aggreggated class, the accessor methods of the aggregated class should be used.
//		-	For an object of the aggregated class to access the data of the aggregating class, the THIS pointer can be passed to the methods of aggregated class!!!

//	#3	COPYING BY VALUE

//	#4	Aggregation/Delegation VS Inheritance:
//		Questions to ask:
//		-	Is anythig in the base class that should not be in the derived class? if yes, DO NOT use inheritance!
//		-	Does the class to be create to have more than one of the base? If yes, use AGGREGATION!
//		-	Do you need to inherit from the base class so that you can take advantage of VIRTUAL METHODS to access PROTECTED DATA MEMBERS? If yes, go ahead and use inheritance!

//	#5	PRIVATE INHERITANCE: Inheriting implementation, not the interface (methods/data).
//		-	Methods and data members of the inherited class are inaccessible by the object of the derived class, but the methods of the derived class DO HAVE THE ACCESS to the methods,
//			and data members of the inherited class!!

//	#6	FRIEND CLASSES:
//		-	Friendship allows 2 or more classes to share "private" methods/data, but next rules apply:
//			1)Non-Inheritable:if Class A friends with Class B, and Class C inherets class A pulbicly, the class C does not have access to class B methods/data.
//			2)Non-Transferable:if Class A friends with Class B, and class C friends with class B, the class A is not firneds with class C.
//			3)Non-Commutative:if Class A befriends Class B, the Class B does not befriend class A by default. Thus, Class B has full access to Class A while Class A DOES NOT have full access to Class B 
//		Syntax:
//			class ClassA
//			{
//			public:
//				friend class ClassB;
//			}; //In this example ClassA befriends ClassB thus giving the ClassB full access to members of the ClassA
//	
//	#7	FRIEND FUNCTION:
//		-	Allows the befriended function to access the protected methods and data 
//		-	Declaring a function to be a friend does not give the friend function access to your THIS pointer!
//		-	When the befriended funtion is also overloaded, each version of this function has to be befriended if you want to grant access to each version!

//	#8	OVERLOADING THE INSERTION OPERATOR (<<):
//		
//	NOTES:
//		-	Aggregation is preferred over private inheritance since aggregation hides the details of implementation while inheritance exposes the details of implementation.
