// POLYMORPHISM: (GOLDEN 3: VRITUAL METHODS, TEMPLATES, and MULTIPLE INHERITANCE!!!)
//			1)	What multiple inheritance is and how to use it.
//			2)	What virtual inheritance is and when to use it.
//			3)	What abstract classes are and when to use them.
//			4)	What pure vitual function are.
//	1.	Perlocating Upward (to the base class): 
//			1)	Methods expected to be used by all of the derived class should be perlocated upward to the base class so
//				they are define only once in the base class instead of defining the same method for every derived class!
//			2)	AVOID PERLOCATING METHODS if you don't expect every derived class to use it!
//	2.	Casting Down: 
//			1)	Adding a method that returns an enumerated type in each of the classes. Thus, we can test each object on what type it is and depending
//				on what this method returns, we can decide whether the object has specific functionality or not (if horse, it can't fly; if pegasus, 
//				it can fly!).
//			2)	Thus, once the type is determined, we can call the specific method contained in the derived class only by CASTING DOWN out base type pointer we
//				are using to point to this derived type object.
//	3.	Multiple Inheritance: Deriving a class from more than 1 base class: class DerivedClass: public Base1Class, public Base2Class
//			1)	The ambiguity problem:
//					a)	2 base classes both have a variable with the same name.
//					b)	2 base classes (let's call them the first and second classes) are actually derived class and are derived from the same base class. 
//						Then, when calling a function inhereted from this same base class, how is the compiler supposed to know whether you are calling it
//						through the inheretance of the 1st or 2nd class? see page 464. This problem is resolved by explicitly telling the compiler which
//						class you want to use. For example fist::getAge() or second::getAge().
//	4.	Finally, we can use VIRTUAL INHERITANCE to resolve problem #3.1.b!!! by using keyword "virtual" when inheriting the common base class (see pg 469)
//			1)	However, the if the classes that inheret from the common base class (first class and second class) have the same variables, the class that
//				inherets the class first and the class second should explicitly say from which class to use the variables!
//			2)	USE VIRTUAL INHERITANCE when deriving from the classes that share a BASE CLASS. REMEMBER to initialize the shared BASE CLASS from the most derived
//				class.
//			3)	DO NOT USE MULTIPLE INHERITANCE when single inheritance will do.

//	5.	Abstract Classes and Pure Virtual Functions:
//			A Pure Virtual Function is a Virtual function initialized to 0: virtual void Draw() = 0;
//			An Abstract Class is a Class that contains at least 1 Pure Virtual Function; such Classes:
//				1)	Are illigal to be used to instantiate an Object. - Thus, DO NOT INSTANTIATE OBJECTS USING ABSTRACT CLASSES!
//				2)	If used to DERIVE a CLASS, the DERIVED CLASS must provide definition for the PURE VIRTUAL FUNCTION! - Thus, MAKE SURE TO PROVIDE DEFINITIONS
//					FOR PURE VIRTUAL FUNCTIONS INHERETED FROM THE ABSTRACT CLASSES
//	6. THINGS TO CHECK:
//		1)	Look more into v-pointers!
